...
module RSpec
  ...
  Matchers.define :trigger_num_queries do |expected|
    chain :with do |matcher|
      @matcher = matcher
    end

    match do |block|
      disabled_cache_block = -> { ::ActiveRecord::Base.uncached { block.call } }
      @counter ||= RSpec::SQLimit::Counter[@matcher, disabled_cache_block]
    ...
    def message(expected, counter, negation = false)
      ...
      condition   = negation ? 'not to' : 'to'
      restriction = " that match #{reporter.matcher}" if reporter.matcher

      <<-MESSAGE.gsub(/ +\|/, '')
        |Expected #{condition} run exactly #{expected} queries#{restriction}
        |#{reporter.call}
      MESSAGE
    end
  end
